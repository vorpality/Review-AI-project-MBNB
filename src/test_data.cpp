...
void process_reviews(const std::vector<std::filesystem::path>& review_paths, int local_start, int local_end,TrainingData& model, int& positives, int& negatives, std::mutex& result_mutex) {
 ...
 //Calculates the probability of a review vector being positive/negative, returns a pair of probabilities.
 //The calculation uses ln to avoid underflow (not 100% certain that there is no loss of data via this method)
std::pair<double,double> test_vector(std::vector<int> review_vector, TrainingData& model) {
    ...
    for (int index = 0; index < review_vector.size(); ++index) {
        ...
        //always returns the predicted positives as first part of the pair and predicted negatives as second part of the pair,
        //might be confusing because if the review given is negative, the prediction is correct when pair.second > pair first
        //whereas when the review given is positive, the prediction is correct when pair.first > pair.second
        //this part could use some improvement to be more clear.
    }
