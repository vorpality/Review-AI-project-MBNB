...
private:
    ...
    //Returns the number of keys in A intersect B
    int calculate_unique_keys(std::map<std::string, int> map1, std::map<std::string, int> map2);

    //Calculates two arrays of probabilities for each word in each class P(X)/P(C) and saves them in member variables m_positive_train_files and m_negative_train_files
    ...
    //Calculates and returns entropy for C
    ...
    //Bubble sorts all vectors on descending Information Gain, sheds results based on inline IG ratio found in config.h
    //functions are grouped for cleaniness because sorter.h bubble_sort_on_ig takes 4 arguments 
    void sort_and_shed_results(float ratio);

    //resizes arrays based on inline IG ratio found in config.h
    void shed_results(float ratio);

public:

    //Class constructor, trains model on call so everything is tidy and keeps data as members in case they are needed for future use
    TrainingData(
        const std::map<std::string, int>& positive_word_counts,
        const std::map<std::string, int>& negative_word_counts,
    ...
    //Getters (most of them are not used currently)
    size_t get_vector_size() { return m_word_index_guide.size(); };
    ...
    std::vector<float> get_positive_vector(int index) { return m_positive_probability_vector; };
    std::vector<float> get_negative_vector(int index) { return m_positive_probability_vector; };
