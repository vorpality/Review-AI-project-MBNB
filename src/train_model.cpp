...
//Shuffles files before adding, for randomness, resizes to given number to control the amount of training data will be processed.
//Splits the files to all available threads and launches threads to do processing (first thread launched tracks % in console) (could probably be improved)
...
//Thread function, makes the map for a smaller, assigned part of the array.
...
//Tracker thread function, same as process_files but prints in console the % of files processed
...
// Trains and returns a TrainingData model
