...
//laplacean prior for smoothing
const float LAPLACEAN_PRIOR = 1.0f;
const std::filesystem::path MAIN_DIR = "x64/Debug/includes/aclImdb";

int max_vector_size;

struct Train_results {
    std::unordered_map<std::string, int>* guide;
...
void tokenize_to_dict(std::filesystem::path file_path, std::map<std::string,int> *word_frequency, bool weighted=false) {
    std::unordered_map<std::string, bool> unique_words;
    std::string path_string = file_path.u8string();
    int score_index = path_string.length() - 5;
    int weight = weighted ? int(path_string.at(score_index)) - 48 : 1;
    ...
    while (std::getline(file, word, ' ')) {
        ...
        //dumps empty words (after removal of special characters)
        //dumps words with less than 2 letters

        if (
            word == "" ||
            word.length() < MINIMUM_LETTERS
           ) {
            continue;
        }
...
std::vector<int> get_vector(std::filesystem::path file_path, std::unordered_map<std::string, int>* guide) {
    std::string path_string = file_path.u8string();
    std::ifstream file(file_path);
    std::string word;
    ...
    while (std::getline(file, word, ' ')) {
        ...
        if (word == "" || word.length() < MINIMUM_LETTERS) {
            continue;
        }
...
void frequency_control(int n, int k, std::map<std::string, int>* word_frequency) {
...
Train_results* train(std::filesystem::path dir) {
    std::map<std::string, int>* neg_map = new std::map<std::string, int>;
    std::map<std::string, int>* pos_map = new std::map<std::string, int>;
    std::cout << "\nTraining data not found. \nTraining anew.."<< std::endl;
    Train_results* data = new Train_results();
    ...
    int neg_file_counter = 0;
    for (const auto& dir_entry : std::filesystem::directory_iterator(negative)) {
        if (dis(gen) < SKIPPAGE) {
            tokenize_to_dict(dir_entry, neg_map);
            neg_file_counter++;
        }
    }
    int n = (int)(PN * neg_file_counter);
    int k = (int)(PK * neg_file_counter);
    frequency_control(n, k, neg_map);
    int pos_file_counter = 0;
    for (const auto& dir_entry : std::filesystem::directory_iterator(positive)) {
        if (dis(gen) < SKIPPAGE) {
            tokenize_to_dict(dir_entry, pos_map);
            pos_file_counter++;
        }
    }
    n = (int)(PN * pos_file_counter);
    k = (int)(PK * pos_file_counter);
    frequency_control(n, k, pos_map);
    //calculating word frequency for positive/negative files, used to calculate respective word probabilities.

    float positive_words = 0;
    for (const auto& [word,count] : *pos_map) {
        positive_words += count;
    }

    float negative_words = 0;
    for (const auto& [word, count] : *neg_map) {
    ...
    */
    std::unordered_map<std::string, int>* guide = new std::unordered_map<std::string, int>;

    std::map<std::string, int> size_counter;
    size_counter.insert(pos_map->begin(), pos_map->end());
    size_counter.insert(neg_map->begin(), neg_map->end());
    max_vector_size= size_counter.size()+1;
    size_counter.clear();

    std::vector<float>* P_positive = new std::vector<float>(max_vector_size);
    std::vector<float>* P_negative = new std::vector<float>(max_vector_size);

    int index = 1;
    for (auto element = pos_map->begin(); element != pos_map->end(); element++) {
        //creating vector guide (connects word with index for alignment)
        std::string key = element->first;
        int value = element->second;

        //creating P vector (Respective Probabilities on each index)
        (*P_positive)[index] = (float)(value + LAPLACEAN_PRIOR) / (float)(pos_file_counter + (LAPLACEAN_PRIOR * 2)); // Laplacean prior
        (*P_negative)[index] = LAPLACEAN_PRIOR / (float) (neg_file_counter + LAPLACEAN_PRIOR*2);
        (*guide)[key] = index;

        index++;
    }
    for (auto element = neg_map->begin(); element != neg_map->end(); element++) {
        std::string key = element->first;
        int value = element->second;

        //if word already exists in guide, add it to the respective place
        if (guide->count(key) > 0) {
            (*P_negative)[(*guide)[key]] = (float)(value + LAPLACEAN_PRIOR) / (float)(neg_file_counter + (LAPLACEAN_PRIOR * 2)); // Laplacean prior
        }

        //adding new words
        
        if (guide->count(key) < 1) {
            (*P_negative)[index] = (float)(value + LAPLACEAN_PRIOR) / (float)(neg_file_counter + (LAPLACEAN_PRIOR * 2)); // Laplacean prior
            (*P_positive)[index] = LAPLACEAN_PRIOR / (float)(neg_file_counter + LAPLACEAN_PRIOR * 2);;
            (*guide)[key] = index;
            index++;
        }
    }
    pos_map->insert(neg_map->begin(), neg_map->end());
    max_vector_size = pos_map->size();
    delete pos_map;
    delete neg_map;
    data->guide = guide;
    data->positive = P_positive;
    data->negative = P_negative;
    data->positive_word_count = positive_words;
    data->negative_word_count = negative_words;
    data->positive_file_count = pos_file_counter;
    data->negative_file_count = neg_file_counter;
    data->total_files = pos_file_counter + neg_file_counter;
    return data;
}

// Function to save training data to a file (to not have to re-train every time)
void save_training_data(const Train_results* data, const std::string& filename) {
...
Train_results* load_training_data(const std::string& filename) {
    Train_results* data = new Train_results();
...
int main(int argc, char *argv[])
{
    ...
    Train_results* train_data = is_training_data_available(training_data_file) ?
